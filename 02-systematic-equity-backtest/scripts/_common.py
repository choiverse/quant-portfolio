...
DEFAULT_DATA = ROOT / "data" / "all_stocks_5yr.csv"
...
def ensure_dirs(outdir: str | None = None) -> tuple[Path, Path, Path]:
    ...
    the full 619k-row panel, and the README would then be documenting numbers
    that no longer match the images beside them.
    """
...
def load_panel(path: str):
    """Load the raw file and derive the wide price / return matrices."""
    from quantbt import data as qdata

    raw = qdata.load_prices(path)
    prices = qdata.to_price_matrix(raw, field="close")
    rets = qdata.to_returns(prices)
