...
# This project uses the same Kaggle panel as project 02. Rather than keep a
# second 28 MB copy, look here first and fall back to project 02's data
# directory, so a checkout that already has the file does not need it twice.
...
def load_panel(path: str):
    """Load the raw file and derive the wide log-price / return matrices."""
    from statarb import data as sdata

    raw = sdata.load_prices(path)
    prices = sdata.to_price_matrix(raw, field="close")
    rets = sdata.to_returns(prices)
